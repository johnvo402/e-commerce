...
namespace Payment.Api.Controllers
{
    ...
    public class PaymentsController : ControllerBase
    {
        ...
        public async Task<IActionResult> VnpayReturn([FromQuery] VnpayPayResponse response)
        {
            ...
            if (returnModel.PaymentStatus == "10")
            {
                return Redirect($"{returnUrl}/Checkout?status=cancel");
            }
             else if (returnModel.PaymentStatus == "00")
            {
                return Redirect($"{returnUrl}/Checkout?status=success");
            } else
            {
                return Redirect($"{returnUrl}/Checkout?status=error");
            }
