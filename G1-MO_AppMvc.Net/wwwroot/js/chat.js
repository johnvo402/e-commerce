$(document).ready(function () {
    ...
    function AppViewModel() {
        ...
        self.joinedRoomFn = ko.observable("");
        ...
        self.joinRoom = function (room) {
            connection.invoke("Join", room.name()).then(function () {
                ...
                self.joinedRoomFn(room.fullName());
            ...
            fetch('/api/Rooms')
                ...
                .then(data => {
                    ...
                    for (var i = 0; i < data.length; i++) {
                        self.chatRooms.push(new ChatRoom(data[i].id, data[i].name, data[i].fullName, data[i].avt == null ? "default-avatar.png" : data[i].avt));
                        
                    }

                    if (self.chatRooms().length > 0) {
                        self.joinRoom(self.chatRooms()[0]);
                        self.joinedRoomFn(self.chatRooms()[3]);
                    }
    ...
    // đổi tượng chatRoom
    function ChatRoom(id, name, fullName, avt) {
        var self = this;
        self.id = ko.observable(id);
        self.name = ko.observable(name);
        self.fullName = ko.observable(fullName);
        self.avt = ko.observable(avt);
    }
    // đối tượng user
