...
namespace App.Areas.Identity.Controllers
{
    ...
    public class UserController : Controller
    {
        ...
        [HttpGet("/managebuyer/")]
        public async Task<IActionResult> IndexBuyer([FromQuery(Name = "p")] int currentPage)
        {
        ...
        [HttpGet("/manageseller/")]
        public async Task<IActionResult> IndexSeller([FromQuery(Name = "p")] int currentPage)
        {
            var model = new UserListModel();
            model.currentPage = currentPage;
        ...
        public async Task<IActionResult> AddRoleAsync(string id, AddUserRoleModel model)
        {
            ...
            model.user.Email = model.UserEmail;
        ...
        public async Task<ActionResult> AddClaimAsync(string userid, AddUserClaimModel model)
        {
            ...
            var claims = _contextapp.UserClaims.Where(c => c.UserId == user.Id);
        ...
        {
            var userclaim = _contextapp.UserClaims.Where(c => c.Id == claimid).FirstOrDefault();
        ...
        public async Task<IActionResult> EditClaim(int claimid, AddUserClaimModel model)
        {
            ...
            if (_contextapp.UserClaims.Any(c => c.UserId == user.Id 
        ...
        {
            var listRoles = from r in _contextapp.Roles
                join ur in _contextapp.UserRoles on r.Id equals ur.RoleId
                where ur.UserId == model.user.Id
                select r;

            var _claimsInRole  = from c in _contextapp.RoleClaims
                                 join r in listRoles on c.RoleId  equals r.Id
                                 select c;
            model.claimsInRole = await _claimsInRole.ToListAsync();


           model.claimsInUserClaim  = await (from c in _contextapp.UserClaims
