...
namespace App.Areas.Identity.Controllers
{
    ...
    public class AccountController : Controller
    {
        ...
        // GET: /Account/Login
        [HttpGet("/login/")]
        [Authorize(Policy = "AccessPublicContent")]
        ...
        public async Task<IActionResult> Login(LoginViewModel model, string returnUrl = null)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                var users = await _userManager.FindByEmailAsync(model.UserNameOrEmail);
                if(users != null && users.Status == 0)
                {
                ...
                if (result.Succeeded)
                {
                    ...
                    if (user != null)
                    {
                        ...
                        if (roles.Contains("Administrator"))
                        {
                            ...
                            return RedirectToAction("Index", "Dashboard", new {area = "Admin"}); // Thay thế "Index" và "Admin" bằng đường dẫn và tên trang admin của bạn
                        }
                    }
        ...
        [AllowAnonymous]
        [ValidateAntiForgeryToken]
        public async Task<IActionResult> Register(int ReSeller, RegisterViewModel model, string returnUrl = null)
        {
            returnUrl ??= Url.Content("~/");
            ViewData["ReturnUrl"] = returnUrl;
            if (ModelState.IsValid)
            {
                if(ReSeller == 1)
                {
                    var user = new AppUser { UserName = model.UserName, Email = model.Email, DateCreate = DateTime.Now, Status = 1, FullName = model.FullName, Avt = "default-avatar.png", PhoneNumber = model.Phone, HomeAdress = model.HomeAddess, Gender = model.Gender, BirthDate = model.BirthDay };
                    var result = await _userManager.CreateAsync(user, model.Password);
                    await _userManager.AddToRoleAsync(user, RoleName.Buyer);
                    await _userManager.AddToRoleAsync(user, RoleName.RequestSeller);
                ...
                {
                    var user = new AppUser { UserName = model.UserName, Email = model.Email, DateCreate = DateTime.Now, Status = 1, FullName = model.FullName, Avt = "default-avatar.png", PhoneNumber = model.Phone, HomeAdress = model.HomeAddess };
                    var result = await _userManager.CreateAsync(user, model.Password);
                    await _userManager.AddToRoleAsync(user, RoleName.Buyer);
            ...
            if (ModelState.IsValid)
            {
                ...
                if((externalEmailUser == null) && (externalEmail == model.Email)) 
                {
                    ...
                    if (resultNewUser.Succeeded)
                    {
                        ...
                        await _userManager.AddToRoleAsync(newUser, RoleName.Buyer);
